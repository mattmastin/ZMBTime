...
task :send_hours => :environment do
  ...
  employees.each do |e|
    ...
    internal_time[e.id] = ApplicationController.to_num(ApplicationController.sum_hours(internal))
    
    project_time[e.id] = ApplicationController.to_num(ApplicationController.sum_hours(submissions))
